...
基本流程和CDH3u3是相同的，但是有一些差异我记录下来。
   - 配置文件在/etc/hadoop，包括环境配置脚本.比如hadoop-env.sh
   - sbin目录下面有hadoop集群启动停止工具，libexec目录下面是公用的配置脚本，而bin下面则偏向应用程序。 *NOTE（dirlt）：感觉比较乱*
   - mapred-site.xml中jobtracker地址配置key修改为 mapred.jobtracker.address
   ...
   - 环境变量JAVA_LIBRARY_PATH用来设置native library path, HADOOP_CLASSPATH可以用来设置hadoop相关的classpath（比如使用hadoop-lzo等）
