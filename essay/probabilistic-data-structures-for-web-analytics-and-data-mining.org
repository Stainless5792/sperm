...
这个地方非常巧妙。以第二个sketch为例，最后v和(v+1)会放到一个cell下面，而以第三个sketch为例，最后v,v+1,v+2,v+3会放到一个cell下面。然后再取[c1,c2]的时候，假设有32个sketch的话，那么最下面取的部分就是[(c1 + 31) / 32, c2 / 32]，排除这个部分之后剩余的部分在16-sketch取，有点类似二分法，整个查询是一个树状结构。
