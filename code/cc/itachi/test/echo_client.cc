...
#define ip "127.0.0.1"
  ...
  virtual void onComplete(AsyncContext* ctx_) {
    ...
    if(ctx->action == AA_CONNECT) {
      ...
      SPERM_DEBUG("ready to read...");
      char buf[64 * 1024];
      int size = read(ctx->fd(), buf, sizeof(buf));
      if(size <= 0 ) { // connection close.
        if(size == 0) {
          SPERM_DEBUG("connection#%zu closed", static_cast<size_t>(ctx->id));
        } else {
          SPERM_WARNING("read(%d) failed(%s)", ctx->fd(), SERRNO);
        }
        ctx->release();
        return ;
      }
      SPERM_DEBUG("read size=%d", size);
...
#ifdef TIMER
        SPERM_TRACE("ping-pong delay %.3lf", current_ms - ctx->session_ms);
#endif
      ...
      SPERM_DEBUG("ready to write...");
      int size = write(ctx->fd(), ctx->wbuf + ctx->wpos, kLength - ctx->wpos);
      if(size <= 0 ) { // connection close.
        if(size == 0) {
          SPERM_DEBUG("connection#%zu closed", static_cast<size_t>(ctx->id));
        } else {
          SPERM_WARNING("write(%d) failed(%s)", ctx->fd(), SERRNO);
        }
        ctx->release();
        return ;
      }
      ctx->wpos += size;
      SPERM_DEBUG("written %d, whole %d", size, kLength);
      if(ctx->wpos >= kLength) {
        ctx->wpos = 0;
        ctx->rpos = 0;
        ctx->initNetReadEvent(AA_READ, ctx->fd());
...
int main() {
  ...
  SPERM_TRACE("connections %d start over", kConnectionNumber);
  while(1) {
    if(exit_) {
      itachi.stop();
      break;
