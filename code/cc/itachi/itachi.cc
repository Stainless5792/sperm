...
void AsyncClient::onComplete(AsyncContext* /*ctx*/) {
  SPERM_DEBUG("AsyncClient::onComplete(%p)", ctx);
}
...
void AsyncContext::release() {
  SPERM_DEBUG("AsyncContext::release");
...
int Itachi::start(int cpu_threads, int poll_threads) {
  if(!stop_) { // already started.
    SPERM_WARNING("itachi %p already started", this);
    return -1;
  }
  stop_ = false;
...
int Itachi::launch_(AsyncContext* ctx, bool auto_acquire) {
  if(ctx->client->itachi != this) {
    SPERM_WARNING("ctx->client->itachi(%p)!=this(%p)", ctx->client->itachi, this);
    return -1;
  }
  if(auto_acquire) {
    ctx->acquire();
  ...
  switch(ctx->type) {
    case AsyncContext::CPU:
      SPERM_DEBUG("%p->type==CPU", ctx);
      code = cpu_handler_.push(ctx);
      break;
    case AsyncContext::NET:
      SPERM_DEBUG("%p->type==NET", ctx);
      code = poll_handler_.push(ctx);
      break;
    default:
      SPERM_DEBUG("unknown %p->type(%d)", ctx, ctx->type);
      code = -1;
      break;
  }
  // rollback.
