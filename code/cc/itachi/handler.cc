...
void Handler::start(int threads) {
  ...
  for(int i = 0; i < threads; i++) {
    ...
    if(ret != 0) {
      SPERM_FATAL("pthread_create(%p+%d,%p,%p) failed(%s)",
...
int CpuHandler::push(AsyncContext* ctx) {
  if(itachi_->stop_) {
    SPERM_WARNING("itachi %p already stop", itachi_);
    return -1;
  }
  int id = ctx->thread_hint % threads_;
  SPERM_DEBUG("cpu push ctx(%p) to queue#%d%s", ctx, id,
...
void* CpuHandler::thread_function(int id) {
  while(1) {
    ...
    if(!queue_[id].pop(&ctx, kQueueWaitTimeoutMillSeconds)) {
      SPERM_DEBUG("cpu queue#%d empty", id);
      continue;
    }
...
void PollHandler::init(int threads) {
  ...
  for(int i = 0; i < threads; i++) {
    ...
    if(!loop_[i]) {
      SPERM_FATAL("ev_loop_new(EVBACKEND_EPOLL) failed");
    }
    ev_ref(loop_[i]);
...
int PollHandler::push(AsyncContext* ctx) {
  ...
  int id = ctx->thread_hint % threads_;
  SPERM_DEBUG("poll push ctx(%p) to queue#%d", ctx, id);
  queue_[id].push(ctx);
...
static void net_callback(int revents, void* arg) {
  ...
  if(revents & EV_ERROR) {
    SPERM_FATAL("libev raise EV_ERROR");
  }
  ...
  if(ctx->client->itachi->launch_(ctx, false) == -1) { // !auto_acquire.
    SPERM_FATAL("ctx->client->itachi->_launch(%p) failed", ctx);
  }
}
    ...
    while(ctx) {
      switch(ctx->type) {
        case AsyncContext::NET:
          ...
          break;
        default:
          SPERM_FATAL("unknown %p->type(%d)", ctx, ctx->type);
      }
      ctx = ctx->next;
    }
  } // while(1)
