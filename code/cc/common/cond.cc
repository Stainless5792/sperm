...
Condition::Condition(MutexLock& mutex): mutex_(mutex) {
  ...
  if(ret < 0) {
    SPERM_FATAL("pthread_cond_init(%p) failed(%s)", &cond_, SERRNO2(ret));
  }
}
...
Condition::~Condition() {
  ...
  if(ret < 0) {
    SPERM_FATAL("pthread_cond_destroy(%p) failed(%s)", &cond_, SERRNO2(ret));
  }
}
...
void Condition::wait() {
  ...
  if(ret < 0) {
    SPERM_FATAL("pthread_cond_wait(%p) failed(%s)", &cond_, SERRNO2(ret));
  }
}
...
bool Condition::timedWait(struct timespec* tp) {
  ...
  if(ret < 0 && ret != ETIMEDOUT) {
    SPERM_FATAL("pthread_cond_timedwait(%p,%p) failed(%s)", &cond_, tp, SERRNO2(ret));
  }
  if(ret == ETIMEDOUT) {
    return false;
  }
...
void Condition::notify() {
  ...
  if(ret < 0) {
    SPERM_FATAL("pthread_cond_signal(%p) failed(%s)", &cond_, SERRNO2(ret));
  }
}
...
void Condition::notifyAll() {
  ...
  if(ret < 0) {
    SPERM_FATAL("pthread_cond_broadcast(%p) failed(%s)", &cond_, SERRNO2(ret));
  }
}
// ------------------------------------------------------------
...
bool FutexCondition::wait(int wait_ms) {
  ...
  //SPERM_DEBUG("dec futex counter=%d",n);
  if(n >= 1) { // at least 1 item, so we can use it.
    //SPERM_DEBUG("acquire futex tid=%zu,n=%d",static_cast<size_t>(get_tid()),n);
    return true;
  }
  struct timespec* tp = NULL;
  struct timespec ctp;
  if(wait_ms != -1) {
    //SPERM_DEBUG("futex wait_ms=%d",wait_ms);
...
again:
  ...
  if(errno == EINTR) {
    ...
    anotify_(1, -1);
    //SPERM_DEBUG("futex timeout wait_ms=%d",wait_ms);
    return false;
  }
  assert(errno != EWOULDBLOCK);
  SPERM_WARNING("futex(%p,FUTEX_WAIT,0,%p) failed(%s)", &(lockword_), tp, SERRNO);
  return false;
}
