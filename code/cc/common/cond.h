...
#ifndef __SPERM_CC_COMMON_CONDITION_H__
#define __SPERM_CC_COMMON_CONDITION_H__
...
public:
  FileDescriptorCondition(): epfd_(-1) {
    ...
    if(factory_.create(fds_) != 0) {
      SPERM_FATAL("factory create fds(%p) failed(%s)", fds_, SERRNO);
    }
    set_nonblock(fds_[0]);
    set_nonblock(fds_[1]);
    // fds_[0] for reading.
    ...
    if(epfd_ == -1) {
      SPERM_FATAL("epoll_create(2) failed(%s)", SERRNO);
    }
    memset(&event_, 0, sizeof(event_));
    event_.events = EPOLLIN;
    if(epoll_ctl(epfd_, EPOLL_CTL_ADD, fds_[0], &event_) != 0) {
      SPERM_FATAL("epoll_ctl(%d,EPOLL_CTL_ADD,%d,%p) failed(%s)",
  ...
  bool wait(int wait_ms) {
    ...
    if(errno != EAGAIN && errno != EWOULDBLOCK) {
      SPERM_WARNING("read(%d) failed(%s)", fds_[0], SERRNO);
      return false;
    }
    if(wait_ms <= 0) {
      return false;
    ...
    while(wait_ms > 0) {
      ...
      if(n < 0) {
        SPERM_WARNING("epoll_wait(%d,%p,1,%d) failed(%s)",
  ...
  bool anotify() {
    ...
    if(::write(fds_[1], c, sizeof(c)) != 1) {
      if(errno != EAGAIN && errno != EWOULDBLOCK) {
        SPERM_WARNING("write(%d) failed(%s)", fds_[1], SERRNO);
      }
      return false;
    }
    return true;
...
#endif // __SPERM_CC_COMMON_CONDITION_H__
